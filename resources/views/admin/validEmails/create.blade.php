...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.validEmail.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.valid-emails.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_company') ? 'is-invalid' : '' }}" name="is_company" id="is_company">
                        ...
                        @foreach(App\Models\ValidEmail::IS_COMPANY_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_company', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('street') ? 'is-invalid' : '' }}" name="street" id="street">{{ old('street') }}</textarea>
