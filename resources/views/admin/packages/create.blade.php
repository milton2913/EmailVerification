        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.packages.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('email_verification_limit') ? 'is-invalid' : '' }}" type="number" name="email_verification_limit" id="email_verification_limit" value="{{ old('email_verification_limit', '0') }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_activated_duration') ? 'is-invalid' : '' }}" name="is_activated_duration" id="is_activated_duration" required>
                        ...
                        @foreach(App\Models\Package::IS_ACTIVATED_DURATION_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_activated_duration', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
